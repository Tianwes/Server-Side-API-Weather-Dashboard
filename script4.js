$(document).ready(function () {
    ...
    function searchWeather(name) {
        ...
        $.ajax({
            ...
            getResponses(response.coord.lat, response.coord.lon, response.name);
        });
    }

    function getResponses(lat, lon, cityName) {
        ...
        $.ajax({
            ...
            setResponses(cityOBJMain, forecastOBJ)

            localStorage.setItem("mainCityInfo", JSON.stringify(cityOBJMain));
            localStorage.setItem("forecastInfo", forecastOBJ);
            // console.log(cityOBJMain)
            //  console.log(forecastOBJ)
        });
    };

    function setResponses(cityOBJ, fiveDayOBJ) {
        // var cityStr = JSON.stringify(cityOBJMain)
        
        console.log(cityOBJ)
        // var cityParsed = JSON.parse(localStorage.getItem("cityInfo"))
        console.log(fiveDayOBJ.temp)
        var inputCityInfo = JSON.parse(localStorage.getItem("mainCityInfo"));
        console.log(inputCityInfo)
        var temp = localStorage.getItem(inputCityInfo.temp)

        var card = $("<div>").addClass("card city-div");
        var cardBody = $("<div>").addClass("card-body");
        var cardTitle = $("<h2>").addClass("card-title").text(inputCityInfo.city);
        var currentTemp = $("<p>").addClass("card-temp").text(temp);
        var humidity = $("<p>").addClass("humidity");
        var windSpeed = $("<p>").addClass("wind");
        var uvIndex = $("<p>").addClass("wind");
        $(".city-info-div").append(card.append(cardBody.append(cardTitle, currentTemp, humidity, windSpeed, uvIndex)))
